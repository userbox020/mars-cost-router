...
if (!browser || (!listedFrames && (!Number.isInteger(start) || !Number.isInteger(end))) || !selectedFrames.length || selectedFrames.some(frame => !Number.isInteger(frame) || frame < 0 || frame > 5399) || new Set(selectedFrames).size !== selectedFrames.length) throw new Error('Usage: node capture.mjs --browser <path> --start 0 --end 5399 --frames <directory>, or --frame-list 300,1350,3150,4950.');
...
try {
  ...
  for (const [index, frame] of selectedFrames.entries()) {
    ...
    const expectedSeconds = Math.floor(frame / 30), expectedClock = `${String(Math.floor(expectedSeconds / 60)).padStart(2, '0')}:${String(expectedSeconds % 60).padStart(2, '0')}`, expectedIndex = Math.min(9, Math.floor((frame / 30) < 15 ? 0 : [15,35,55,75,100,120,140,160,170].filter(boundary => frame / 30 >= boundary).length));
